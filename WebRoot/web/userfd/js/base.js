...
(function($) {
	$.fn.jdSlide = function(option) {
		...
		var init = function() {
			var img = "<ul style='position:absolute;top:0;left:0;'><li><a href='"
					+ settings.pics[0].href
					+ "' target='_blank'><img src='"
					+ settings.pics[0].src
					+ "' width='"
					+ settings.width
					+ "' height='" + settings.height + "' /></a></li></ul>";
			element.css({
				"position" : "relative"
			}).html(img);
			$(function() {
		...
		var initIndex = function() {
			...
			for ( var i = 0; i < total; i++) {
				...
				;
				indexs += "<span class='" + current + "'><a href='"
						+ settings.pics[i].href + "' target='_blank'>" + x
						+ "</a></span>";
			}
			;
			indexs += "</div>";
		...
		var running = function(index, tag) {
			...
			;
			element.find("span").eq(settings.index).removeClass(
					settings.current);
		...
		var delayLoad = function() {
			...
			for ( var i = 1; i < total; i++) {
				img += "<li><a href='" + settings.pics[i].href
						+ "' target='_blank'><img src='" + settings.pics[i].src
						+ "' width='" + settings.width + "' height='"
						+ settings.height + "' /></a></li>";
			}
			;
...
gi_rotate__.deliver = function(arg) {
	...
	click += 'onclick="javascript:window.open(\'' + ad_arr[i].gi_ldp
			+ '\',\'_blank\')"></div>';
...
function compare() {
	if ($("#compare").get(0) == null) {
		$("body")
				.append(
						"<div id=\"compare\" class=\"compare\"><div class=\"mt\"><h5>��Ʒ�Ƚ�</h5><div class=\"extra\" onclick=\"clearCompare()\"></div></div><div class=\"comPro\"><ul class=\"mc\" id=\"comProlist\"></ul><div class=\"mb\"><input type=\"button\" value=\"�Ա���ѡ��Ʒ\" class=\"btn\" id=\"compareImg\" onclick=\"openCompare()\"></div></div></div>");
		$("#compare").css({
			position : "absolute",
			top : "220px",
			right : "0px"
		...
		if (proIDs.length < 3) {
			...
			$("#comProlist")
					.append(
							"<li id='check_"
									+ checkid
									+ "'><a title='ɾ��' class='close' onclick='reduceCompare("
