/**
 * 
 * Description 分页插件 Powered By 小K QQ 908526866 E-mail lnend@sina.cn Data
 * 2013-05-28 Dependence jQuery v1.8.3
 * 
 */
	...
	$.fn.kkPages = function(options) {
		...
		return this
				.each(function() {
					...
					// 定义分页导航
					var PagesNavHtml = "<div class=\"Pagination\"><a href=\"javascript:;\" class=\"homePage\">首页</a><a href=\"javascript:;\" class=\"PagePrev\">上一页</a><span class=\"Ellipsis\"><b>...</b></span><div class=\"pagesnum\"></div><span class=\"Ellipsis\"><b>...</b></span><a href=\"javascript:;\" class=\"PageNext\">下一页</a><a href=\"javascript:;\" class=\"lastPage\">尾页</a></div>";
					...
					if ($AllMth > $Mth) {
						...
						// 判断显示
						var relMth = $Mth - 1;
						$this.find($PagesClass).filter(":gt(" + relMth + ")")
								.hide();

						// 计算数量,页数
						var PagesMth = Math.ceil($AllMth / $Mth); // 计算页数
						var PagesMthTxt = "";
						$this.append(PagesNavHtml).find(".Pagination").append(
								PagesMthTxt);
						...
						for ( var i = 1; i <= PagesMth; i++) {
							...
							PagesNavNum = PagesNavNum
									+ "<a href=\"javascript:;\">" + i + "</a>";

						}
						;

						$this.find(".pagesnum").append(PagesNavNum).find(
								"a:first").addClass("PageCur");
						...
						$PagesPrev.click(function() {
							...
							var OldNav = $this
									.find(".pagesnum a[class=PageCur]");

							if (OldNav.text() == 1) {
								alert("亲：不能再点啦~已经是首页啦！");
							} else {
						...
						function showPages(page) {
							...
							$PagesNav.each(function() {
								...
								if (NavText == page) {
									...
									$(this).addClass("PageCur").siblings()
											.removeClass("PageCur");
